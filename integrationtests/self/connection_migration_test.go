...
func TestNewPreferredAddress(t *testing.T) {
	serverConnV4, err := net.ListenUDP("udp", net.UDPAddrFromAddrPort(netip.MustParseAddrPort("127.0.0.1:9443")))
	require.NoError(t, err)
	// defer serverConnV4.Close()
	serverConnV6, err := net.ListenUDP("udp", net.UDPAddrFromAddrPort(netip.MustParseAddrPort("[::1]:9443")))
	...
	conn, err := tr1.Dial(ctx, net.UDPAddrFromAddrPort(netip.MustParseAddrPort("127.0.0.1:9443")), getTLSClientConfig(), getQuicConfig(&quic.Config{
		EnableNewPreferredAddress: true,
	}))
	require.NoError(t, err)
	defer conn.CloseWithError(0, "")
	...
	err = sconn.NewPreferredAddress(netip.AddrPort{}, preferredAddr)
	require.NoError(t, err)

	time.Sleep(3 * rtt) // wait for preferred address to filter through
